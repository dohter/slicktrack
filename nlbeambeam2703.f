...
C   It is assumed that the horizontal sigma is greated than the vertical sigma.
...
C     WRITE(53,'(A,2E16.6)')' Coeff1, Coeff2:   ',COEFF1,COEFF2


      ACOEFF1 = AGP1*COEFF1
      ACOEFF2 = AGP1*COEFF2

      SORBVEC(2,IP) = SORBVEC(2,IP) + COEFF1
      SORBVEC(4,IP) = SORBVEC(4,IP) + COEFF2
C
      IF(NSOLBB.GT.0)GO TO 9
      SORBVEC(7,IP)=   ACOEFF1*LZI 
     +               - ACOEFF2*LXI
     +               + SORBVEC(7,IP)
      SORBVEC(8,IP)= - ACOEFF1*MZI 
     +               + ACOEFF2*MXI 
     +               + SORBVEC(8,IP)
    9 CONTINUE  
