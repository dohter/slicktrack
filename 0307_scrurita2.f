...
C      PSCALE(1:NSECT) = PSCALE(1:NSECT)/RADAVE  ! Scale factors for individual dipoles.
...
C      WRITE(*,'(A,I10)')'Reached NLBEAMBEAM 1 ',MODIBMBM
...
C      IDAMPFLG = 1                              !Switch on/off radn.
C      TH = 0.5D0                                !Top hat from -0.5 to +0.5
C      VARTH = TH*TH/3.D0                        !The standard value for a top hat distn.
...
C      PHOTSCALE = CTIME*SCALEMAT(6,6)/(TDAMP(5)*VARTH*(NSECT-1))
C      PHOTSCALE = 2.D0*DSQRT(PHOTSCALE*NDAMP2)*IDAMPFLG
C      TH = TH * PHOTSCALE
...
C      WRITE(*,'(A,I10)')'Reached NLBEAMBEAM 2 ',MODIBMBM
...
C      SORBVEC(1:6,1:NPART2) = 
C     +                MATMUL(SECTMAP(1:6,1:6,II),SORBVEC(1:6,1:NPART2))
...
C      DO 3347 I=1,8
C 3347 WRITE(53,9144)(SECTMAP(I,J,II),J=1,8)
9144  FORMAT(8F20.12)

      GMAT = SECTMAP(7:8,1:6,II)
      SORBVEC(7:8,1:NPART2) = MATMUL(GMAT,SORBVEC(1:6,1:NPART2))
     +                      + SORBVEC(7:8,1:NPART2) 
...
C      IF(II.EQ.180)
C      WRITE(53,'(A,2I10,4E24.5)')'SECT,IDNT ',
C     +                             II,IDNT,
...
C      WRITE(53,'(A,I10,E16.5)')' PSCALE:  ',II,PSCALE(II)
      ...
      DO 55 IPHS = 1,NPART2
C      SORBVEC(6,IPHS) = SORBVEC(6,IPHS) + BIGPHOT(IPHS) * PSCALE(II)
...
C     rates obtained with the ``used'' damping time.
     ...
     +                                               DFITA(1)/DFITB(1)
      ...
      WRITE(53,103)
      WRITE(53,'(A,A,3D15.4)')' Total:  fitted slope (+error), offset',
