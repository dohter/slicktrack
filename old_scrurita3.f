...
C 
      ...
      DIMENSION SPINKICK1(3,NPART),SPINKICK2(3,NPART),SPINKICK3(3,NPART)
      ...
      DIMENSION SPINVET1(NPART,3),SPINVET2(NPART,3),SPINVET3(NPART,3)
...
C=====8x8 eigenvector stuff.
      ...
      DIMENSION SPINVECQ(2,NPART),SPINVETQ(NPART,2)
      DIMENSION COVSPINQ(2,2,MAXT)
      DIMENSION SANG1(NPART),SANG2(NPART),SANG3(NPART)
      DIMENSION SDIFFUSIONQ(MAXT),    DFITQ(2),    DVECQ(2)
      DIMENSION SDIFFUSIONQA(MAXT),   DFITQA(2),   DVECQA(2)
      DIMENSION SDIFFUSIONQB(MAXT),   DFITQB(2),   DVECQB(2)
      DIMENSION SDIFFUSIONQAB(MAXT),  DFITQAB(2),  DVECQAB(2)
...
C     +                      ' using sections.','   NDAMP3= ',NDAMP3
...
C   Renormalise as in SPRINT. RB(2) is always zero here. So ignore it.
      ...
      RB(1)  = RB(1)/RBNORM
      RB(3)  = RB(3)/RBNORM
C   Now get the quarternions for the spin field vectors starting with the initial
...
C   Now get the total kick for a section and construct its unit quarternion.      
C      SANG1(1:NPART3) = MATMUL(GMAT9(1,:),SORBVEC(1:6,1:NPART3))    !SLIM's alpha
C      SANG2(1:NPART3) = MATMUL(GMAT9(2,:),SORBVEC(1:6,1:NPART3))    !SLIM's beta 
C      SANG3(1:NPART3) = MATMUL(GMAT9(3,:),SORBVEC(1:6,1:NPART3)) 
...
C   The angle SPINKICK(3, ) (``gamma'') is a +ve rotation around n0.
      DO 777 IQQ = 1,NPART3
      RB(0) =  1.0D0
...
C   Renormalise as in SPRINT. 
      RBNORM = DSQRT(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))  
      RB(0)  = RB(0)/RBNORM
      RB(1)  = RB(1)/RBNORM
      RB(2)  = RB(2)/RBNORM
      RB(3)  = RB(3)/RBNORM
...
C      REDSPIN(:,INPR) = SORBVEC(7:8,INPR) - ENVEC(:,INPR)
      REDSPIN(:,INPR) = SPINKICKA(1:2,INPR) - ENVEC(:,INPR)
   67 CONTINUE  
      ENDIF
...
C   Renormalise again (needed?).
      DO 91 IQQQ = 1,NPART3
      RQNORM = DSQRT(RQA(0,IQQQ)*RQA(0,IQQQ) 
     +            +  RQA(1,IQQQ)*RQA(1,IQQQ) 
     +            +  RQA(2,IQQQ)*RQA(2,IQQQ) 
     +            +  RQA(3,IQQQ)*RQA(3,IQQQ))  
...
C
      RQNORM = DSQRT(RQ1(0,IQQQ)*RQ1(0,IQQQ) 
     +            +  RQ1(1,IQQQ)*RQ1(1,IQQQ) 
     +            +  RQ1(2,IQQQ)*RQ1(2,IQQQ) 
     +            +  RQ1(3,IQQQ)*RQ1(3,IQQQ))  
      RQ1(0,IQQQ)  = RQ1(0,IQQQ)/RQNORM
      RQ1(1,IQQQ)  = RQ1(1,IQQQ)/RQNORM
      RQ1(2,IQQQ)  = RQ1(2,IQQQ)/RQNORM
      RQ1(3,IQQQ)  = RQ1(3,IQQQ)/RQNORM
      SPINVEC1(2,IQQQ) 
     +      = 2.D0*(RQ1(1,IQQQ)*RQ1(2,IQQQ) - RQ1(0,IQQQ)*RQ1(3,IQQQ)) !beta along l
      SPINVEC1(1,IQQQ) 
     +      = 2.D0*(RQ1(3,IQQQ)*RQ1(2,IQQQ) + RQ1(0,IQQQ)*RQ1(1,IQQQ)) !alpha along m
C
      RQNORM = DSQRT(RQ2(0,IQQQ)*RQ2(0,IQQQ) 
     +            +  RQ2(1,IQQQ)*RQ2(1,IQQQ) 
     +            +  RQ2(2,IQQQ)*RQ2(2,IQQQ) 
     +            +  RQ2(3,IQQQ)*RQ2(3,IQQQ))  
      RQ2(0,IQQQ)  = RQ2(0,IQQQ)/RQNORM
      RQ2(1,IQQQ)  = RQ2(1,IQQQ)/RQNORM
      RQ2(2,IQQQ)  = RQ2(2,IQQQ)/RQNORM
      RQ2(3,IQQQ)  = RQ2(3,IQQQ)/RQNORM
      SPINVEC2(2,IQQQ) 
     +      = 2.D0*(RQ2(1,IQQQ)*RQ2(2,IQQQ) - RQ2(0,IQQQ)*RQ2(3,IQQQ)) !beta along l
      SPINVEC2(1,IQQQ) 
     +      = 2.D0*(RQ2(3,IQQQ)*RQ2(2,IQQQ) + RQ2(0,IQQQ)*RQ2(1,IQQQ)) !alpha along m
C
      RQNORM = DSQRT(RQ3(0,IQQQ)*RQ3(0,IQQQ) 
     +            +  RQ3(1,IQQQ)*RQ3(1,IQQQ) 
     +            +  RQ3(2,IQQQ)*RQ3(2,IQQQ) 
     +            +  RQ3(3,IQQQ)*RQ3(3,IQQQ))  
      RQ3(0,IQQQ)  = RQ3(0,IQQQ)/RQNORM
      RQ3(1,IQQQ)  = RQ3(1,IQQQ)/RQNORM
      RQ3(2,IQQQ)  = RQ3(2,IQQQ)/RQNORM
      RQ3(3,IQQQ)  = RQ3(3,IQQQ)/RQNORM
      SPINVEC3(2,IQQQ) 
     +      = 2.D0*(RQ3(1,IQQQ)*RQ2(3,IQQQ) - RQ3(0,IQQQ)*RQ3(3,IQQQ)) !beta along l
      SPINVEC3(1,IQQQ) 
     +      = 2.D0*(RQ3(3,IQQQ)*RQ2(3,IQQQ) + RQ3(0,IQQQ)*RQ3(1,IQQQ)) !alpha along m
C
  91  CONTINUE 
...
C===Get the covariance matrices and the 3-D spin info.
     ...
     +        REDSPINT(1:NPART3,:)   = TRANSPOSE(REDSPIN(:,1:NPART3))
      COVTRACK(:,:,IDNT) 
     +       = MATMUL(SORBVEC(:,1:NPART3),SORBVET(1:NPART3,:))/NPART3
      COVSPINA(:,:,IDNT)
     +    = MATMUL(SPINVECA(:,1:NPART3),SPINVETA(1:NPART3,:))/NPART3
      COVSPIN1(:,:,IDNT)
     +    = MATMUL(SPINVEC1(:,1:NPART3),SPINVET1(1:NPART3,:))/NPART3
...
C   We could also just use the n0 component directly but we would be burying 
C   information that could be useful for diagnostics. 
C   Wait for renormalisation until the end of a turn. 
      DO 91 IQQQ = 1,NPART3
      RQNORM = DSQRT(RQA(0,IQQQ)*RQA(0,IQQQ) 
     +            +  RQA(1,IQQQ)*RQA(1,IQQQ) 
     +            +  RQA(2,IQQQ)*RQA(2,IQQQ) 
  ...
  91  CONTINUE 
      ...
      SPINVETQ(1:NPART3,:)   = TRANSPOSE(SPINVECQ(:,1:NPART3))
      COVSPINQ(:,:,IDNT)
     +        = MATMUL(SPINVECQ(:,1:NPART3),SPINVETQ(1:NPART3,:))/NPART3
C      SDIFFUSION(IDNT)      = COVSPINA(1,1,IDNT)+COVSPINA(2,2,IDNT)
      SDIFFUSIONQ(IDNT)      = COVSPINQ(1,1,IDNT)+COVSPINQ(2,2,IDNT)
      SDIFFUSIONQA(IDNT)     = COVSPINQ(1,1,IDNT)
      SDIFFUSIONQB(IDNT)     = COVSPINQ(2,2,IDNT)
      SDIFFUSIONQAB(IDNT )   = COVSPINQ(1,2,IDNT)
...
C      SORBVEC(7:8,1:NPART3)  = MATMUL(TW2A,SORBVEC(7:8,1:NPART3))
      SPINVECA(:,1:NPART3)   = MATMUL(TW2A,SPINVECA(:,1:NPART3))
      SPINVEC1(:,1:NPART3)   = MATMUL(TW2A,SPINVEC1(:,1:NPART3))
      SPINVEC2(:,1:NPART3)   = MATMUL(TW2A,SPINVEC2(:,1:NPART3))
      SPINVEC3(:,1:NPART3)   = MATMUL(TW2A,SPINVEC3(:,1:NPART3))
C   Wind back RQ in an analogous way. 
  ...
  956 FORMAT(T6,2F16.3)
   ...
   89 FORMAT(/,' The ',I3,'th  transported spin covariance matrix',
     ...
     +          ' at the 1-st beam line element after ',I6,' turns')
      ...
      WRITE(53,80)INT,IDNT
   80 FORMAT(/,' The ',I3,'th  transported spin covariance matrix',
     +          ' mode-A(Q) <Si*Sj>(mrad*mrad...)',
     +          ' at the 1-st beam line element after ',I6,' turns')
      WRITE(53,956)COVSPINQ(:,:,IDNT)*1.D6
...
C====Now do fits. Skip data from the first few damping times.
      ...
      DVECQ(1)   = DVECQ(1)   + JF*SDIFFUSIONQ(JF)
      DVECQ(2)   = DVECQ(2)   +    SDIFFUSIONQ(JF)
      DVECQA(1)  = DVECQA(1)  + JF*SDIFFUSIONQA(JF)
      DVECQA(2)  = DVECQA(2)  +    SDIFFUSIONQA(JF)
      DVECQB(1)  = DVECQB(1)  + JF*SDIFFUSIONQB(JF)
      DVECQB(2)  = DVECQB(2)  +    SDIFFUSIONQB(JF)
      DVECQAB(1) = DVECQAB(1) + JF*SDIFFUSIONQAB(JF)
      DVECQAB(2) = DVECQAB(2) +    SDIFFUSIONQAB(JF)
  ...
  106 CONTINUE 
     ...
     +          (-DIFFMAT(2,1)*DVECAB3(1)+DIFFMAT(1,1)*DVECAB3(2))/DET
      ...
      DFITQ(1)  =( DIFFMAT(2,2)*DVECQ(1)   - DIFFMAT(1,2)*DVECQ(2))/DET
      DFITQ(2)  =(-DIFFMAT(2,1)*DVECQ(1)   + DIFFMAT(1,1)*DVECQ(2))/DET
      DFITQA(1) =( DIFFMAT(2,2)*DVECQA(1)  - DIFFMAT(1,2)*DVECQA(2))/DET
      DFITQA(2) =(-DIFFMAT(2,1)*DVECQA(1)  + DIFFMAT(1,1)*DVECQA(2))/DET
      DFITQB(1) =( DIFFMAT(2,2)*DVECQB(1)  - DIFFMAT(1,2)*DVECQB(2))/DET
      DFITQB(2) =(-DIFFMAT(2,1)*DVECQB(1)  + DIFFMAT(1,1)*DVECQB(2))/DET
      DFITQAB(1)=
     +          ( DIFFMAT(2,2)*DVECQAB(1) - DIFFMAT(1,2)*DVECQAB(2))/DET
      DFITQAB(2)=
     +          (-DIFFMAT(2,1)*DVECQAB(1) + DIFFMAT(1,1)*DVECQAB(2))/DET
...
C      IF(E0.EQ.9.45D0)THEN
     ...
     +                      SDIFFUSIONQ(KF),  DFITQ(1)*KF   +DFITQ(2),
     +                      SDIFFUSIONQA(KF), DFITQA(1)*KF  +DFITQA(2),
     +                      SDIFFUSIONQB(KF) ,DFITQB(1)*KF  +DFITQB(2),
     +                      SDIFFUSIONQAB(KF),DFITQAB(1)*KF +DFITQAB(2),
...
C      RATEMC = 0.5D0*DFIT(1)/CTIME/NDAMP3
      RATEMC = 0.5D0*DFITQ(1)/CTIME/NDAMP3
