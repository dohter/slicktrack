C   15/11/79 511212038  MEMBER NAME  TSPIN    (SEPT95.S) M  FORTRAN
      SUBROUTINE QSPIN(II,IID,ITY,XXX,XX2,YYY,S,C)
C
C     Adapted from TSPIN. 
C=====Routine, adapted from TSPIN to set up unit quarternions in the l,n0,m 
C     frame for nonzero orbit amplitudes.

C     As a first step we describe spin rotations as kicks as in SLIM and use 
C     the existing expressions of TSPIN. 
...
C     IF(NTWIST(ITY).EQ.4)DDX=DX(II)-TWIST(ITY)
    ...
    6 CALL RSPIN(0.D0,-XXX1+GG,XXXS*GGY,C)
      RETURN
    7 CALL RSPIN(+XXX1-GG,0.D0,-XXXS*GGX,C)
      RETURN
...
C      Sept 2003: fix this so that the C.O. effects can be killed.
      ...
      DYPA = DYPA                                    *1.D0
      CALL RSPIN(XXXS*DXPA,XXXS*DYPA,-XXX+GG,B)
      CALL JAM333(A,B,C)
      FDX = DX(II+1)                                 *1.D0 
      FDY = DY(II+1)                                 *1.D0 
