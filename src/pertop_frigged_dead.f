...
C
      ...
      DIMENSION INTGE6(6),WW6(6,6),ZZ(6,6),ZZNC(6,6),TM6NCA(6,6)
      DIMENSION WR(6),WI(6),TUN(6),VC6A(6),VC6B(6),EIG6(6,6)
      DIMENSION TRIN66(6,6),RRNC6(6),RINC6(6),VRNC6(6,6),VINC6(6,6)
      DIMENSION WRNC(6),WINC(6),TUNNC(6),TM6NCB(6,6)
...
C
  ...
  555 CONTINUE
      CALL UCOPY(TREV6,TM6B,72)
C     CALL EIV6(TREV6,TM6A,WR,WI,IERRO)
C     IF(IERRO .NE. 0) GOTO 9999
C=====USE THE NAG ROUTINE. THIS IS MORE ROBUST THAN EIV6.
      CALL UCOPY(TREV6,TRIN6,72)
      IFAIL=0
      CALL F02AGF(TRIN6,6,6,RR6,RI6,VR6,6,VI6,6,INTGE6,IFAIL)
      IF(IFAIL.NE.0)GO TO 9999
      CALL UCOPY(TREV66,TRIN66,72)
c======Repeat for case of no cavities.
      IFAIL=0
      CALL F02AGF(TRIN66,6,6,RRNC6,RINC6,VRNC6,6,VINC6,6,INTGE6,IFAIL)
      IF(IFAIL.NE.0)GO TO 9999
...
C9266 FORMAT(' ','NOW THE F02AGF RESULTS: 8X8')
 ...
 2822 CONTINUE
      ...
      WR(I)  =RR6(I)
 2844 WI(I)  =RI6(I)
      CALL UCOPY(ZZ,TM6A,72)
      CALL UCOPY(ZZ  ,TM6NCA,72)
C
C======Get re-ordered eigenvectors and tunes.
      CALL RENORM(TM6A,6,WR,WI)
      CALL RENORM(TM6NCA,6,WRNC,WINC)
C
C
...
C=====                                    I.E. AT START OF ELEMENT II
      ...
      JTY=ITYPE(II-1)
      IID=ID(JTY)
      IF(IID.NE.5.AND.IID.NE.6.AND.IID.NE.7)S=S+YY(JTY)
      ...
      BETACX(II) = 2.D0*     (TM6NCA(1,1)**2 + 0.D0*TM6NCA(1,2)**2)
      BETACY(II) = 2.D0*     (TM6NCA(3,3)**2 + 0.D0*TM6NCA(3,4)**2)
      DO 566 I=1,4
      VC4A(I)=TM6(I,6)
      DO 566 J=1,4
...
C=====TEST FOR SYMPLECTICITY
      ...
      WRITE(53,9155)
      DO 388 I=1,6
      TUNNC(I)=DATAN2(WINC(I),WRNC(I))/PI2
      DAMPNC=-DLOG(DSQRT(WRNC(I)*WRNC(I)+WINC(I)*WINC(I)))
  388 WRITE(53,916)WRNC(I),WINC(I),(TM6NCB(J,I),J=1,6),TUNNC(I),DAMPNC
...
C
  110 DO 116 I=1,3
     ...
     +                                           PDIF(J,I)=PDIF(J,I)+PI200010959
      IF(PTEMP(J,I).LT.P(J,I).AND.DABS(PTEMP(J,I)-P(J,I)).GT.PI)
     +                                           PDIF(J,I)=PDIF(J,I)-PI200010961
