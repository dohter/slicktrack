...
C
      SUBROUTINE NLBEAMBEAM(II,X,X2,YY,S,ZWBB,SORBVEC,NSOLBB,MODIBMBM)

C    II,XXBB(II),X2BB(II),YYBB(II),NU,ZWBB,SORBVEC,NSOLBB(II),MODIBMBM)
...
C     USE EITHER ROUND       GAUSSIAN BEAMS (IBMBM = +/-3) OR 
C                ELLIPTICAL  GAUSSIAN BEAMS (IBMBM = +/-2) WITH EXACT CALC OR
...
C   Even with the mixture of magnetic fields, the spin kick can 
C   be derived from the orbit kick in the usual way.
C   It is assumed that the hor. sigma is greated than the verti. sigma.
      ...
      WRITE(53,'(A,A,A)')
     +  ' In NLBEAMBEAM: ',
     +  'the horizontal size of oncoming beam is less than or the same', 
     +  ' as the vertical size. So STOP.'
      STOP
      ENDIF
      ...
      Z1 = DEN * DCMPLX(SORBVEC(1,IP),   SORBVEC(3,IP)   )
      Z2 = DEN * DCMPLX(SORBVEC(1,IP)*R1,SORBVEC(3,IP)*R2)
      EXPFAC =  DEXP(-RSQ1*S1 - RSQ2*S2)
      IFAIL = 0      
      W1 = S15DDF(Z1, IFAIL)
      W2 = S15DDF(Z2, IFAIL)    

      DELTA  = CONSTE * (EXPFAC * W2 - W1) 
      COEFF1 = DELTA                       ! Get the real part
      COEFF2 = DIMAG(DELTA)                ! Get the imaginary part

      ACOEFF1 = AGP1*COEFF1
      ACOEFF2 = AGP1*COEFF2

      SORBVEC(2,IP) = SORBVEC(2,IP) + COEFF1*SORBVEC(1,IP)
      SORBVEC(4,IP) = SORBVEC(4,IP) + COEFF2*SORBVEC(3,IP)
C
      IF(NSOLBB.GT.0)GO TO 9
      SORBVEC(7,IP)=   ACOEFF1*SORBVEC(1,IP)*LZI 
     +               - ACOEFF2*SORBVEC(3,IP)*LXI
     +               + SORBVEC(7,IP)
      SORBVEC(8,IP)= - ACOEFF1*SORBVEC(1,IP)*MZI 
...
C
      ...
      SORBVEC(7,IP)=   ACOEFF*( SORBVEC(1,IP)*LZI - SORBVEC(3,IP)*LXI)
     +               + SORBVEC(7,IP)
      SORBVEC(8,IP)=   ACOEFF*(-SORBVEC(1,IP)*MZI + SORBVEC(3,IP)*MXI)
     +               + SORBVEC(8,IP)
   10 CONTINUE  
C
      ENDIF
