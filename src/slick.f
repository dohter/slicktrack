...
C    *************** Fanglei uncomments on 12May20 ********
      ...
      OPEN(56,FILE=GAUSSES,      STATUS='UNKNOWN')
...
C=====CHANGE ROTATOR SETTINGS AT EACH ENERGY.
  ...
  611 CONTINUE
      ...
      CALL CAVITY(E0,IE0,U0,RLENGE,CRAD)
      U0KEEP=U0
C
C      IF(1.EQ.1)STOP
...
C     IF IPTCO=0 THE C.O. WAS NEVER CALCULATED AND IS THUS ZERO ANYWAY.
...
C      IF(ISECT.EQ.0)CALL SPIN(IE0,E0,CIR,TAUY,BETAY0,TUN,NCRAZY)
C     IF(1.EQ.1)STOP

C     Rerun PERTOP, DAMPER, EMITNC at each energy to ensure that SCRURITA3 has the right input.
      CALL PERTOP(E0,IE0,U0,CIR,TM6A,TUN)
      CALL DAMPER(E0,IE0,U0,CRAD,CIR,TDAMP,TUN,ANTIDP)
      IF(ANTIDP.LT.0)THEN
      WRITE(53,1063)
