C   15/11/79 510241814  MEMBER NAME  SOL8AN   (S)           FORTRAN
      SUBROUTINE SOL9AN(I,X,Y,SS,ZW,A,B,NSOL)
C
C
...
C     9x9 version of SOL8AN. See MX99DAMP for background.
...
C
      ...
      DATA ICALL/0/
      DIMENSION ZW(3,3),A(3,3),T1(3,3),T2(3,3),T3(3,3)
      DIMENSION B(9,9),T5(9,9),T4(9,9),T6(4,4)
      DIMENSION RSOL(3)
C
C
      INCLUDE "cloorb.for"
...
C
      ...
      CALL RSPIN(XX2*DX(I)*.5/Y,XX2*DY(I)*.5/Y,0.D0,T1)
      ...
      B(7,1)= .5D0*XX1*T3(1,3)/Y
      B(7,3)= .5D0*XX1*T3(2,3)/Y

      B(8,1)=-.5D0*XX1*T3(1,2)/Y
      B(8,3)=-.5D0*XX1*T3(2,2)/Y

      B(9,1)= .5D0*XX1*T3(1,1)/Y
      B(9,3)= .5D0*XX1*T3(2,1)/Y
   ...
   15 CONTINUE
      ...
      XDELA=X*(DEL(I)+DEL(I+1))*.5D0            *1.D0 ! Switch of C.O. effects Sept 2003
      DXPA = DXPA                               *1.D0
      DYPA = DYPA                               *1.D0
...
C
      ...
      T4(9,6)= T2(3,1)*(1.D0 +0.0011596)*X
   ...
   16 CONTINUE
      CALL JAM999(T5,T4,B)
      CALL RSPIN(-XX2*DX(I+1)*.5/Y,-XX2*DY(I+1)*.5/Y,0.D0,T1)
      CALL JAM333(A,T1,T3)
      CALL AVER(T3,A,3,T1)
      CALL UNIT(9,T4)
      T4(2,3)=-.5D0*X/Y
      T4(4,1)=-T4(2,3)
      T4(7,1)=-.5D0*XX1*T1(1,3)/Y
      T4(7,3)=-.5D0*XX1*T1(2,3)/Y

      T4(8,1)= .5D0*XX1*T1(1,2)/Y
      T4(8,3)= .5D0*XX1*T1(2,2)/Y

      T4(9,1)=-.5D0*XX1*T1(1,1)/Y
      T4(9,3)=-.5D0*XX1*T1(2,1)/Y
