C   02/09/86 609111505  MEMBER NAME  ORCORR   (S)           FORTRAN
...
C
      ...
      DIMENSION WIN(2,24),ALAG(2,24),WPU(2,24),KON(2,24),Q(2)
C
C
...
C=====CORRECTIONS AFTER SETTING UP THE PHASE DIFFERENCES ETC.
...
C
  103 CONTINUE
      WRITE(6,61)ALAG(KXZ,J2),ALAG(KXZ,J3),J1,J2,J3,Q(KXZ)
...
C
C
      Y1=B(1,J1)*B(3,J2)*DPU1
      Y2=B(1,J1)*B(3,J3)*DPU2+B(1,J2)*B(3,J3)*DPU3*R21
      IF(WPU(KXZ,J2)+WPU(KXZ,J3).LT.0.001)GO TO 1
      A1=-(Y1*ALAG(KXZ,J2)*WPU(KXZ,J2)+Y2*ALAG(KXZ,J3)*WPU(KXZ,J3))
      A1=A1/(Y1*Y1*WPU(KXZ,J2)+Y2*Y2*WPU(KXZ,J3))
C     A1=-ALAG(KXZ,J2)/Y1
C     A1=0.003
      A2=A1*R21
      A3=A1*R31
...
C=====ACCUMULATED PREDICTED NEW ORBIT.
      ALAG(KXZ,J2)=ALAG(KXZ,J2)+Y1*A1
      ALAG(KXZ,J3)=ALAG(KXZ,J3)+Y2*A1
      WRITE(6,51)ALAG(KXZ,J2),ALAG(KXZ,J3),J1,J2,J3
