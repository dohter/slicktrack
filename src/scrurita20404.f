...
C 
      ...
      PARAMETER (NFFT    = 10000-1) ! Maximum number of turns for the FFT of the orbit.
      REAL*8     NU
...
C=====8x8 eigenvector stuff.
      ...
      DIMENSION XFFT(0:NFFT-1), WXFFT(NFFT), YFFT(0:NFFT-1),WYFFT(NFFT)
      DIMENSION AXFFT(0:NFFT-1),BXFFT(NFFT),AYFFT(0:NFFT-1),BYFFT(NFFT)
C
...
C
     ...
     +                       DAMPTURNS,'  NDAMP2=', NDAMP2
      ...
      IDAMPFLG = 1                              !Switch on/off radn.
      TH = 0.5D0                                !Top hat from -0.5 to +0.5
      ...
      TH = TH * PHOTSCALE
      WRITE(53,'(A,A,A,I6,3E15.7)')' ','NSECT, PHOTSCALE,', 
...
C      Use just 1 particle and sample after the first 5 damping times.   
       IF(IE0.EQ.1.AND.INT.GT.5)THEN 
      IF(IFFT.LE.NFFT)THEN 
      XFFT(IFFT) = SORBVEC(1,1)
      YFFT(IFFT) = SORBVEC(3,1)    
C      WRITE(53,'(A,3I10,2F16.6)')' Reached here 2: ',
C     +                       IE0,INT,IFFT,XFFT(IFFT),YFFT(IFFT)
      IFFT = IFFT + 1
...
C====At the end of each damping time, write out sample info
  ...
  956 FORMAT(T6,2F16.3)
   ...
   89 FORMAT(/,' The ',I3,'th  transported spin covariance matrix',
     ...
     +                         COVTRACK(7,7,IDNT)/COVTRACK(8,8,IDNT)
...
C===Do the FFT's
      ...
      CALL C06FAF(XFFT,KIFFT,XWFFT,IFAIL)
      IF(IFAIL.NE.0)THEN
      WRITE(53,103)
      WRITE(53,103)
      WRITE(53,'(A,A,A,I3)')
