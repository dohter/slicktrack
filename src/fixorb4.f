      SUBROUTINE FIXORB4(E0,IE0,U0,CRAD,FREQ,KKICK)
C
C
C
...
C      So constrain the size of the correction kicks. But that alone gives
C      This is similar to FIXORB2 but concentrates on minimising the vertical kicks
C      once FIXORB1 has done a first sufficient orbit correction.
C
C
C      Care with the signs of kicks is more essential than usual.
...
C======Storage for the least squares NAG routine.
      DIMENSION A(NM,NC),AUSE(2*NM,NC),C(NC,1),
     +        BMEAS(2*NM,1),BQUAD(2*NM,1),BQUADP(2*NM,1),
     +        QR(2*NM,NC), ALPHA(NC),
     ...
     +        WORK(LWORK), BTEMP(2*NM,1)
      ...
      DIMENSION AUPPER(NM,NC),WKSPACE(NC),ATEMP(NM,NC)
C
C
C
      INCLUDE  "cdesy2.for"
...
C
  ...
  103 FORMAT(/,/,/,/,/,'  ')
      WRITE(53,'(A,A,A,A)')
     +   '1',' FIXORB4: Correct the combined vertical kicks of',
...
C      QCOMB = QCOMB + X2(KTY)
 ...
 67   CONTINUE
      ...
      A(NVM,NVCCNT) = (BQUADP(NVM,1)-BQUAD(NVM,1))/DELXX
...
C      The response matrix must be completed to include the contributions of the
C      correctors themselves by adding a unit matrix.
      CALL UNIT(NM,AA)
      AUPPER = 0.D0
      AUPPER(1:NVM,1:NVCCNT) = A(1:NVM,1:NVCCNT) - AA(1:NVM,1:NVCCNT)
...
C======For fitting use AUSE instead of A or AUPPER. This allows some monitors to be
C      switched off by killing lines in AUSE.
C      But we need A later to look at the residuals at the dead monitors to see if they are
C      indeed large.

      AUSE = 0.D0
C======Include a constraint to limit the size of the correction kicks so as to try to
C      control the inherent instability of kick minimisation.
C      Constrain the total strength of each VC.
      CALL UNIT(NM,AA)
      CONSTRAINT2 = 0.D-2
      AUSE(NVM+1:2*NVM,1:NVCCNT) = AA(1:NVCCNT,1:NVCCNT)*CONSTRAINT2
      BMEAS(NVM+1:2*NVM,1)       = -XXKEEP(1:NVM)*CONSTRAINT2
...
C      i.e. there are still NVCCNT correctors.
      ...
      DO 42 IG = 1,NVM
  42  AUSE(IG,1:NVCCNT) = G9(IG)* AUPPER(IG,1:NVCCNT)
