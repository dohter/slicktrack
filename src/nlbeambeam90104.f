...
C
  ...
  103 FORMAT(/,'  ')
      WRITE(53,'(A)') 
     +  ' NLBEAMBEAM: oncoming beam size is zero in at least one plane.'      
      STOP
      ENDIF
...
C   It is assumed that the horizontal sigma is greated than the vertical sigma.
     ...
     +  ' same as the vertical size. So STOP.'
      ...
      IFAIL = 0
      DO 9 IP = 1,NPART2 
      RSQ1     = SORBVEC(1,IP)**2 *1.D-0
      RSQ2     = SORBVEC(3,IP)**2 
      RSQ      = RSQ1 + RSQ2
      IF(RSQ.EQ.0.D0)GO TO 9

      Z1 = DEN * DCMPLX(SORBVEC(1,IP)*1.D-0,   SORBVEC(3,IP)   ) 
      Z2 = DEN * DCMPLX(SORBVEC(1,IP)*R1*1.D-0,SORBVEC(3,IP)*R2) 
