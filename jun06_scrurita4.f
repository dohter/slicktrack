...
C
C
C   Routine to handle spin-orbit tracking on beam lines and estimate the rate of 
C   depolarisation
...
C====Naive spin tune.
      NU=E0/0.440652D0 * 1.0D-0       ! Can scale a gamma indep. of the energy.
...
C
C----------------------------------------------------------------------------------
C   Put in the covariance matrix by hand.
...
C      COVMATIN(6,6) = 0.01
      ...
      COVMATIN(1,1) = 0.029379D0**2
      COVMATIN(2,2) = 0.001575D0**2
      COVMATIN(3,3) = 0.000863D0**2
      COVMATIN(4,4) = 0.000096D0**2
      COVMATIN(5,5) = 0.3D0**2
      COVMATIN(6,6) = 0.6D0**2
      COVMATIN(1,2) = 0.913659D0*DSQRT(COVMATIN(1,1)*COVMATIN(2,2))
      COVMATIN(2,1) = COVMATIN(1,2)
      COVMATIN(3,4) =-0.540759D0*DSQRT(COVMATIN(3,3)*COVMATIN(4,4))
      COVMATIN(4,3) = COVMATIN(3,4)


Cx [mm]         0.000000     0.029379
Cpx [mrad]      0.000000     0.001575     0.913659
Cy [mm]         0.000000     0.000863     0.000000     0.000000
Cpy [mrad]      0.000000     0.000096     0.000000     0.000000    -0.540759
Ct [mm]         0.000000     0.300000     0.000000     0.000000     0.000000     0.000000
Cpt [mrad]      0.000000     0.600000     0.000000     0.000000     0.000000     0.000000     0.000000
...
C      IF(IE0.EQ.1)
      CALL G05CBF(KSEED15)       !Seed for initial s-o distribution. Original = 15 
...
C======Get the 1-pass spin-(symplectic) orbit matrix again.
    ...
    8 CONTINUE
      ...
      WRITE(53,103)
      WRITE(53,'(A)')
     +   ' 1-pass spin-(symplectic)orbit matrix, generated afresh.'
...
C======Get the 1-pass spin-(damped) orbit matrix.
  ...
  888 CONTINUE
      ...
      WRITE(53,103)
      WRITE(53,103)
      WRITE(53,'(A,/,A,I1,A)')' 1-pass spin-(damped)orbit matrix',
     +                               ' NDAMP3 = ',NDAMP3, ' !!!!!!!'
      DO 336 I=1,8
...
C      IF(IE0.EQ.1)
      CALL G05CBF(KSEED16)             !Set the seed for the big photons.
...
C    and only if the section is not nonlinear a beam-beam element.
...
C      WRITE(*,'(A,3I10)') 'Reached here 5', IDT,INT,II
      DO 90 IK = 1,6
...
C====At the end of the line write out sample info
   ...
   85 FORMAT(//,' The ',I3,'th  transported s-o covariance matrix',
     +          ' <Xi*Xj>(mm*mm,mm*mrad...)',
     +          ' at the 1-st beam line element after ',I6,' pass')
