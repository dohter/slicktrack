...
C 
      ...
      DIMENSION SDIFFUSION13(MAXT),   DFIT13(2),   DVEC13(2)
      DIMENSION SDIFFUSIONA13(MAXT),  DFITA13(2),  DVECA13(2)
      DIMENSION SDIFFUSIONB13(MAXT),  DFITB13(2),  DVECB13(2)
      DIMENSION SDIFFUSIONAB13(MAXT), DFITAB13(2), DVECAB13(2)

      DIMENSION SDIFFUSION23(MAXT),   DFIT23(2),   DVEC23(2)
      DIMENSION SDIFFUSIONA23(MAXT),  DFITA23(2),  DVECA23(2)
      DIMENSION SDIFFUSIONB23(MAXT),  DFITB23(2),  DVECB23(2)
      DIMENSION SDIFFUSIONAB23(MAXT), DFITAB23(2), DVECAB23(2)

      DIMENSION SDIFFUSION12(MAXT),   DFIT12(2),   DVEC12(2)
      DIMENSION SDIFFUSIONA12(MAXT),  DFITA12(2),  DVECA12(2)
      DIMENSION SDIFFUSIONB12(MAXT),  DFITB12(2),  DVECB12(2)
      DIMENSION SDIFFUSIONAB12(MAXT), DFITAB12(2), DVECAB12(2)
...
C      DIMENSION RNORMA(NPART),RNORM1(NPART),RNORM2(NPART),RNORM3(NPART)
C      DIMENSION RNORM13(NPART),RNORM23(NPART),RNORM12(NPART)
C
...
C===Set initial unit quarternion.
  ...
  103 FORMAT(/,'  ')
      ...
      NQOFF = 0                 !Switch on/off single and combined mode calcs.
      WRITE(53,929)NQOFF
...
C      RNORMA=1.D0; RNORM1=1.D0; RNORM2=1.D0; RNORM3=1.D0 !Reset quarternion norms
C      RNORM13=1.D0; RNORM23=1.D0; RNORM12=1.D0
...
C   The angle SPINKICK(3,:) ( no name ) is a -ve rotation around n_0.
...
C      RNORMA(IQQ) = 
C     +     RNORMA(IQQ)*(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))
...
C      RNORM1(IQQ) = 
C     +     RNORM1(IQQ)*(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))
...
C      RNORM2(IQQ) = 
C     +     RNORM2(IQQ)*(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))
...
C    Combined modes 1 and 3.
      ...
      RNORM13(IQQ) = 
     +    RNORM13(IQQ)*(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))
...
C    Combined modes 2 and 3.
      ...
      RNORM23(IQQ) = 
     +    RNORM23(IQQ)*(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))
...
C    Combined modes 1 and 2.
      ...
      RNORM12(IQQ) = 
     +    RNORM12(IQQ)*(RB(0)*RB(0)+RB(1)*RB(1)+RB(2)*RB(2)+RB(3)*RB(3))
...
C     a turn as in SPIN, but only after logging the spin info.
      ...
      CALL QUARTM(RQ13(:,IQQQQ),TWQ,RQ13(:,IQQQQ))
      CALL QUARTM(RQ23(:,IQQQQ),TWQ,RQ23(:,IQQQQ))
      CALL QUARTM(RQ12(:,IQQQQ),TWQ,RQ12(:,IQQQQ))
      ENDIF
  333 CONTINUE 
...
C      RATEMC = 0.5D0*DFIT(1)/CTIME/NDAMP3
     ...
     +                     'Mode-3: fitted slope and offset: ',DFIT3 
      ...
      WRITE(53,'(A,A,2D15.8,A)')' ', 
     +                     'Mode-1,3: fitted slope and offset: ',DFIT13 
      ...
      WRITE(53,'(A,A,2D15.8,A)')' ', 
     +                     'Mode-2,3: fitted slope and offset: ',DFIT23 
      ...
      WRITE(53,'(A,A,2D15.8,A)')' ', 
     +                     'Mode-1,2: fitted slope and offset: ',DFIT23 
      WRITE(53,'(A,A,D15.4,A)')' ', 'M-C TAUD12  ', TAUDMC12(IE0),' SEC'

      ENDIF

      WRITE(63,'(I10,20E16.5)')IE0,NU,TAUDMC(IE0)  ,RATEMC,
     +                                 TAUDMC1(IE0) ,RATEMC1, 
     +                                 TAUDMC2(IE0) ,RATEMC2, 
     +                                 TAUDMC3(IE0) ,RATEMC3,
     +                                 TAUDMC13(IE0),RATEMC13,
     +                                 TAUDMC23(IE0),RATEMC23,
     +                                 TAUDMC12(IE0),RATEMC12,E0 
      ENDIF
