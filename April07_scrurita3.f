...
C======WIND BACK THE SPIN BASIS==========================================
     ...
     +           ' using sections.', ' NDAMP3 = ',NDAMP3, ' !!!!!!!'
      ...
      DO 35 I=1,9
   35 WRITE(53,9150)(AAA9(I,J),J=1,9)
...
C    PHOTSCALE should be close to the sqrt of NDAMP3.
...
C      WRITE(53,'(A,2I10)')'II, IDNT ', II,IDNT
...
C      IF(IBBSECT(II).NE.1.OR.(IBBSECT(II).EQ.1.AND.MODIBMBM.LT.2))
C     +          SORBVEC(1:6,1:NPART3) = 
C     +                MATMUL(SECTMAP(1:6,1:6,II),SORBVEC(1:6,1:NPART3))
...
C      WRITE(*,'(A,I10)')'Reached here 2 ',II
...
C      WRITE(53,'(A,I10,E16.5)')' PSCALE:  ',II,PSCALE(II)
      ...
      DO 55 IPHS = 1,NPART3
C      WRITE(53,'(A,I10,E20.10)')'Bigphot :',IDNT,BIGPHOT(1)      
...
C      Use the file: BEAMDIST. 
      ...
      NSPIN = NSPIN + 1
      SPINANG      = DATAN2(SPINVECA(1,IQQQ),SPINVECA(2,IQQQ))
...
C      WRITE(*,'(A,3I10,F20.6)')' Reached here,',
C     +                     NSPIN,NBIN,KBIN(NBIN),SPINANG
      ENDIF
      ENDIF
...
C      WRITE(*,'(A,2I10)')'Reached here 2',  INT,IDNT
...
C      WRITE(*,'(A,2I10)')'Reached here             1',  INT,IDNT
...
CC      DO 102 JS = 1,100
CC  102 WRITE(59,'(1X,I10,1X,I10,1X,I10)')JS,KBIN(JS)
CC      SPINAVE      =  SPINAVE/NSPIN
CC      SPINSIG      =  SPINSIG/NSPIN
CC      SPINSPREAD   =  DSQRT(SPINSIG - SPINAVE**2)
CC      SPINSPREAD   =  SPINSPREAD * 180.D0/pi
CC      SPINAVE      =  SPINAVE * 180.D0/pi
CC      DECOHANG     =  NU * DSQRT(SCALEMAT(6,6))/DABS(TN(5)) * 180.D0/pi
CC      WRITE(53,103)
CC      WRITE(53,103)
CC      WRITE(53,'(2A,I6,2F10.4,A)')
CC     + ' The number of spins, the spin spread in the alpha-beta plane',
CC     + ' and a model prediction: ',
CC     + NSPIN,SPINSPREAD,DECOHANG,' degrees'
C----------------------------------------------------------------------------------------
